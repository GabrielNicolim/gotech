...
main()
{
	...
	srand(time(NULL));
	random_menu = rand() % 4; // Defines the art that will appear in the main menu
...
void registerProductData()
{
	...
	product.id = IDaux;									// Após validação toda copia para a struct
	strcpy(product.name, validateName(39, 10));			// Recebe nome e já copia para a struct
	product.quantity = validateQuantity(39, 12);		// Recebe quantidade e já copia para a struct					
	product.type = validateType(39, 14);		 		// Recebe tipo e já copia para a struct
	product.price = validatePrice(39, 16);	 			// Recebe preco e já copia para a struct
...
{	
	...
	while (true)
	{
		...
		if(isIDRegistered(searchID)){			
			...
			while(!feof(fp) && fread(&product, sizeof(product), 1, fp))
			{
...
{	
	...
	int contl = 1, limite, limiteAnte, pag, linha, pag_limite;	
	char retornar;
	...
	do{
		...
		{			
			...
			else if(product.type == aux) contl++;
		}
...
int searchByName(const char* name) {
    ...
    while (fread(&product, sizeof(product), 1, fp) == 1) {
        if (stristr(product.name, name) != NULL) results++;
    }
...
void queryName()
{
	...
	while (true)
	{
		...
		if(results == 0)
		{
			textcolor(CONTRAST_COLOR);
			gotoxy(69, 18);
			printf("[ Nome n%co encontrado ]", 198);
		...
		int cont_tuplas = results; // Contador de registros 
		char retornar = '0';
		
		cursor(0);

		int contl = 1, limit, limiteAnte, pag, linha, pageLimit;
		
		pag = 1;
		
		pageLimit = ceil(cont_tuplas / 12);  
		
		generateTable(20);
		
		do {
					
			limit = (12 * pag); // 12 linhas de dados por página (oq cabe na tabela)
...
void deleteData()
{
	...
	while (true) {
		...
		if(!isIDRegistered(IDaux)) {
			...
			while (fread(&product, sizeof(Product), 1, fp) == 1) {
				if (product.id == IDaux){ 
					textcolor(TEXT_COLOR); textbackground(RED);
					gotoxy(20, 27);
